...
boards = os.listdir(f"{basedir}/2020.10")
...
def plot_set_remove():
    ...
    for version, board in itertools.product(["2020.10", "pr13103"], boards):
        filename = f"{basedir}/{version}/{board}/tests_xtimer_benchmarks/xunit.xml"
        root = ET.parse(filename)
        path = f"testcase[@classname='tests_xtimer_benchmarks.Timer Overhead']//property"
        properties = [
            p
...
def plot_timer_now():
    ...
    for version, board in itertools.product(["2020.10", "pr13103"], boards):
        filename = f"{basedir}/{version}/{board}/tests_xtimer_benchmarks/xunit.xml"
        root = ET.parse(filename)
        path = f"testcase[@classname='tests_xtimer_benchmarks.Timer Overhead'][@name='Measure Overhead TIMER_NOW']//property"
